...
CREATE TABLE IF NOT EXISTS warning_history (id INTEGER PRIMARY KEY AUTOINCREMENT, guild_id INTEGER NOT NULL, user_id INTEGER NOT NULL, moderator_id INTEGER NOT NULL, reason TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
...
def init_db():
    with sqlite3.connect(DATABASE_PATH) as conn:
        ...
        guild_ids = [row[0] for row in conn.execute("SELECT DISTINCT guild_id FROM tickets").fetchall()]
        for guild_id in guild_ids:
            rows = conn.execute("SELECT id FROM tickets WHERE guild_id=? AND ticket_number IS NULL ORDER BY id", (guild_id,)).fetchall()
            next_number = conn.execute("SELECT COALESCE(MAX(ticket_number), 0) FROM tickets WHERE guild_id=?", (guild_id,)).fetchone()[0] or 0
            for row in rows:
                next_number += 1
                conn.execute("UPDATE tickets SET ticket_number=? WHERE id=?", (next_number, row[0]))
        conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_tickets_guild_number ON tickets(guild_id, ticket_number)")
        conn.commit()
@contextmanager
def connection():
...
def set_guild_data(guild_id,data):
    with connection() as conn:
        conn.execute("INSERT INTO guild_settings(guild_id,data) VALUES(?,?) ON CONFLICT(guild_id) DO UPDATE SET data=excluded.data",(guild_id,json.dumps(data,ensure_ascii=False)))
def update_guild_data(guild_id,**changes):
    data=get_guild_data(guild_id); data.update(changes); set_guild_data(guild_id,data)
    return data
